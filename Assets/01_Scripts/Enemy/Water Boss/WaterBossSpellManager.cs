...
public class WaterBossSpellManager : MonoBehaviour
{
    ...
    private void SpawnSpellLeft(int indexVFX)
    {
        ...
        {
            Vector3 relativePos = _waterBossAgentController.PlayerTransform.position - _waterBossAgentController.WaterBossTransform.position;
    ...
    private void SpawnOnPlayer(int indexVFX)
    {
        ...
        if (_firepointLeft != null)
        {
            ...
            // the second argument, upwards, defaults to Vector3.up
            Quaternion rotation = Quaternion.LookRotation(relativePos, Vector3.up);
            vfx = Instantiate(_vfxs[indexVFX], _waterBossAgentController.PlayerTransform.position, rotation);
        }
        else
        {
            Debug.Log("No Fire Poinr");
    ...
    {
        _waterBossAnimatorMono.SetSpell(valueBool, valueInt);
